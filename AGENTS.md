...
### Interactive and autonomous remote plan / apply

Backend coordinates (`TF_CLOUD_HOSTNAME`, `TF_CLOUD_ORGANIZATION`, `TF_WORKSPACE`)
are pulled from OpenBao and dynamically generated by the shared `nix-devenv` helper
at `direnv allow`. The operator's shell must export `BAO_ADDR`,
`OPENBAO_APPROLE_TERRAFORM_ROLE_ID`, and `OPENBAO_APPROLE_TERRAFORM_SECRET_ID` before
loading the worktree (sourced from Doppler T3 / the auto-locking keychain).

Authentication uses the `~/.terraform.d/credentials.tfrc.json` token generated
by a human running `tofu login` once per machine. **AI agents inherit this session**
and require no further login prompts.
...
> **AI agents**: After acquiring initial permission to run commands, an agent
> may run `tofu init`, `plan`, and `apply` autonomously for the duration of the
> session, relying on the pre-existing `tofu login` token.
