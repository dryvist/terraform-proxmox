...
# 198.18.20.0/24), injected by terragrunt.hcl and shared one-way with
...
variable "network_cidrs" {
  description = "Map of VLAN name => network-form CIDR (e.g. siem => 198.18.20.0/24). Sourced from Doppler NETWORK_CIDR_* (sensitive). IPs are derived via cidrhost(cidr, vm_id); masks are taken from the CIDR itself."
  type        = map(string)
  sensitive   = true
  ...
  validation {
    ...
    error_message = "Each network_cidrs entry must be a valid network-form CIDR (e.g. 198.18.20.0/24)."
  }
}
